...
class ReinforceNMT(FairseqCriterion):
    ...
    def __init__(self, task, max_len_decoding, max_order):
        ...
        self.scorer = bleu.Scorer(pad=tgt_dict.pad(), eos=tgt_dict.eos(), unk=tgt_dict.unk())
    ...
    def add_args(parser):
        ...
        # fmt: off
        parser.add_argument('--max-len-decoding', default=100, type=int, metavar='D',
    ...
    def forward(self, model, sample, reduce=True, generator=None):
        ...
        #     src_tokens = torch.tensor([sample['net_input']['src_tokens'][i][:]])
        #     src_lengths = torch.tensor([sample['net_input']['src_lengths'][i]])
        ...
        loss = utils.move_to_cuda(rewards_baseline_detached - rewards_detached) * lprobs_added
        loss = loss.sum()
