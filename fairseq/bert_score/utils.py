...
from transformers import AutoModel, GPT2Tokenizer, AutoModelForSeq2SeqLM
...
model2layers = {
    ...
    "NlpHUST/t5-vi-en-base": 6,
    "haotieu/en-vi-mt-model": 6,
}
...
def get_model(model_type, num_layers, all_layers=None):
    ...
    elif model_type.endswith('t5-vi-en-base'):
    ...
    if not all_layers:
        if hasattr(model, "n_layers"):  # xlm
            ...
            if hasattr(model.encoder, "albert_layer_groups"):
                ...
                assert (
                    0 <= num_layers <= len(model.encoder.layers)
                ), f"Invalid num_layers: num_layers should be between 0 and {model.encoder.layers} for {model_type}"
                model.encoder.layers = torch.nn.ModuleList([layer for layer in model.encoder.layers[:num_layers]])
            else:  # bert, roberta
                assert (
